...
*/
    ...
    private void insertAndUpdateDB() {
        Cursor cursor = getBaseContext().getContentResolver().query(Articles.CONTENT_URI, null, null, null, Articles._ID+" DESC");
        if(cursor.getCount() >= 40){
            if (cursor.moveToFirst()) {
                int row = getApplicationContext().getContentResolver().delete(Articles.CONTENT_URI, Articles._ID + "=?", new String[]{String.valueOf(40)});
            ...
            getApplicationContext().getContentResolver().insert(Articles.CONTENT_URI, values);
        }
    }
